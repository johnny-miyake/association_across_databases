...
In `app/models/user_action.rb:2`, `establish_connection :log_db` means  `UserAction` model corresponds to `user_action` table on the database which is specified as `log_db` section in `config/database.yml`.
